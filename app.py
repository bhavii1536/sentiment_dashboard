...
if menu == t(lang,"creator_insights","Content Creator Insights"):
    ...
    if st.button(t(lang, "analyze_channel","Analyze Channel"), use_container_width=True) and channel_id.strip():
        ...
        for item in yt_comments:
            ...
            sent = analyze_sentiment(cleaned, lg)
            rec = {
                "text": item["text"], "clean": cleaned, "lang": lg, "sent": sent,
                "src": "YouTube", "videoTitle": item.get("videoTitle",""), "views": item.get("viewCount",0)
            }
            records.append(rec)
        ...
        colA.metric(t(lang,"results","Total Comments"), f"{total}")
        colB.metric(t(lang,"positive","Positive %"), f"{(pos/total)*100:0.1f}%")
        colC.metric(t(lang,"neutral","Neutral %"), f"{(neu/total)*100:0.1f}%")
        colD.metric(t(lang,"negative","Negative %"), f"{(neg/total)*100:0.1f}%")

st.write("")
st.caption("Tip: Set your YT API key as env var YT_API_KEY. Twitter uses snscrape fallback (no API key needed).")
